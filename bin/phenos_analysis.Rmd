...
rename_dict <- c('ibdmix' = 'IBDMix', 'argweaverd' = 'ARGweaver-D', 'archaicseeker' = 'ArchaicSeeker2', 'archie' = 'ArchIE',
                 ...
                 'sankararaman2016_1' = 'Sankararaman16 (1)', 'sankararaman2016_2' = 'Sankararaman16 (2)', 'skov2020' = 'hmmix20',
                 'skov2018' = 'hmmix18', 'steinruecken2018' = 'DICAL-ADMIX', 'iasi_2024' = 'admixfrog')

# set home dir
HOMEDIR="/wynton/home/capra/ychen39/"

# method directories vector
methods_dirs = c("li_2024", "yuan_2021", "durvasula_2019", "steinruecken_2018", "skov_2020", "schaefer_2021", "hubisz_2020", "browning_2018_updated_filtering", "vernot_2016", "sankararaman_2014", "sankararaman_2016_1", "sankararaman_2016_2", "iasi_2024", "skov_2018")
...
# remove the overlap_11_high_support_no_archie_argweaver, overlap_11_high_support_with_archie_argweaver, overlap_12_no_archie_argweaver, overlap_14_high_support_with_archie_argweaver, and archie/argweaverd
introgressed_list <- introgressed_list[!sapply(introgressed_list, function(x) endsWith(x, ' overlap_11_high_support_no_archie_argweaver_GREAT_merged.bed'))]
introgressed_list <- introgressed_list[!sapply(introgressed_list, function(x) endsWith(x, 'overlap_11_high_support_with_archie_argweaver_GREAT_merged.bed'))]
introgressed_list <- introgressed_list[!sapply(introgressed_list, function(x) endsWith(x, 'overlap_12_no_archie_argweaver_GREAT_merged.bed'))]
introgressed_list <- introgressed_list[!sapply(introgressed_list, function(x) endsWith(x, 'overlap_14_methods_with_archieargweaver_GREAT_merged.bed'))]
#introgressed_list <- introgressed_list[!sapply(introgressed_list, function(x) endsWith(x, 'archie_GREAT_merged.bed'))]
#introgressed_list <- introgressed_list[!sapply(introgressed_list, function(x) endsWith(x, 'argweaverd_GREAT_merged.bed'))]
introgressed_list <- introgressed_list[!sapply(introgressed_list, function(x) endsWith(x, 'overlap_11_high_support_no_archie_argweaver_GREAT_merged.bed'))]
introgressed_list <- introgressed_list[!sapply(introgressed_list, function(x) endsWith(x, 'overlap_12_high_support_no_archie_argweaver_GREAT_merged.bed'))]
introgressed_list <- introgressed_list[!sapply(introgressed_list, function(x) endsWith(x, 'overlap_14_high_support_with_archie_argweaver_GREAT_merged.bed'))]
```
...
#     # get region-gene associations
     genes_associated[[introgressed_method]] = as.data.frame(region_gene_list[[introgressed_method]])
 }
...
names(all_phenos_list) <- sub("/", "", sub(GREAT_input_dir, "", sub("_GREAT_merged.bed$", "", names(all_phenos_list))))
 names(genes_associated) <- sub("/", "", sub(GREAT_input_dir, "", sub("_GREAT_merged.bed$", "", names(all_phenos_list))))

# all phenotypes tested for each map in a separate all_phenos dataframe
all_phenos <- bind_rows(all_phenos_list, .id = "method")

# save region-gene associations to supplement file
for (map in names(genes_associated)) {
  outfile <- file.path(as.character(paste0(HOMEDIR, '/introgression_methods/cleaned/supplement_data/regions_to_genes/', map, '_regions_to_genes.tsv')))
  
  df <- as.data.frame(genes_associated[[map]])
  df[] <- lapply(df, function(x) {
  if (is.list(x)) {
    sapply(x, function(y) paste(y, collapse = ","))  # collapse vectors in each cell
  } else {
    x  # leave atomic vectors as-is
  }
})
  
  write.table(df,
              file = outfile,   # filename from name
              sep = "\t",                 # tab-separated
              quote = FALSE,
              row.names=FALSE)              # don’t quote strings
}

genes_associated_final <- map_dfr(genes_associated, ~., .id = "map")

write_tsv(genes_associated_final, file = paste0(HOMEDIR, '/introgression_methods/cleaned/supplement_data/rGREAT_maps_to_genes.tsv'))
```
...
# remove archie and argweaver
sig_phenos <- sig_phenos[sig_phenos$method != "argweaverd", ]
sig_phenos <- sig_phenos[sig_phenos$method != "archie", ]
...
top_phenos_df <- top_phenos_df %>%
  mutate(highlight = ifelse(id %in% sig_list_11_or_more_no_archie_argweaverd$id, "highlight", "normal"))
...
ggplot(top_phenos_df, aes(y = id, x = as.factor(method))) +
  ...
  scale_y_discrete(labels = function(x) str_to_sentence(tolower(x))) +
  ...
  labs(
    ...
    color = "Odds Ratio",
    size = "Overlap",
    title = "Significant Phenotypes Supported by 3+ Methods"
  ) +
...
### 11+ methods overlapping - No ArchIE or Argweaver
```{r}
# run rGREAT
res_list_11_or_more = run_GREAT_GO(input_file_bed_path="/wynton/home/capra/ychen39/introgression_methods/data/GREAT_analysis/input/overlap_11_high_support_no_archie_argweaver_GREAT_merged.bed",
                                               GO_type='msigdb:C5:HPO')

# get enrichment table / sig phenotypes table
tb_11_or_more = getEnrichmentTable(res_list_11_or_more) #Occasionally, the output p-values will be 0 since the lowest pvalue that GREAT can return is 1E-324. Enrichments more significant than this will show a pvalue of 0.
sig_list_11_or_more_no_archie_argweaverd = tb_11_or_more[tb_11_or_more$p_adjust < 0.05,]

# edit names so they don't contain file path and suffix
names(sig_list_11_or_more_no_archie_argweaverd) <- sub("/", "", sub(GREAT_input_dir, "", sub("_GREAT_merged.bed$", "", names(sig_list_11_or_more_no_archie_argweaverd))))

# number of sig phenotypes
# how many of these sig phenotypes are supported by 3+ separately (top_phenos)
paste0("Number sig phenotypes:", nrow(sig_list_11_or_more_no_archie_argweaverd), ", ", sum(!is.na(match(unique(top_phenos), unique(sig_list_11_or_more_no_archie_argweaverd$id)))), " of these are supported by 3+ separately")

# add to genes_associated
region_gene_list[['11_or_more_no_archie_argweaver']] = getRegionGeneAssociations(res_list_11_or_more)
genes_associated[['11_or_more_no_archie_argweaver']] = as.data.frame(region_gene_list[['11_or_more_no_archie_argweaver']])

```

### 11+ methods overlapping - including archie and argweaverd
```{r}
# run rGREAT
res_list_11_w_archieargweaverd = run_GREAT_GO(input_file_bed_path="/wynton/home/capra/ychen39/introgression_methods/data/GREAT_analysis/input/overlap_11_high_support_with_archie_argweaver_GREAT_merged.bed",
                                               GO_type='msigdb:C5:HPO')

# get enrichment table / sig phenotypes table
tb_11_w_archieargweaverd = getEnrichmentTable(res_list_10) #Occasionally, the output p-values will be 0 since the lowest pvalue that GREAT can return is 1E-324. Enrichments more significant than this will show a pvalue of 0.
sig_list_11_w_archieargweaverd = tb_11_w_archieargweaverd[tb_11_w_archieargweaverd$p_adjust < 0.05,]

# edit names so they don't contain file path and suffix
names(sig_list_11_w_archieargweaverd) <- sub("/", "", sub(GREAT_input_dir, "", sub("_GREAT_merged.bed$", "", names(sig_list_11_w_archieargweaverd))))

# number of sig phenotypes
# how many of these sig phenotypes are supported by 3+ separately (top_phenos)
paste0("Number sig phenotypes:", nrow(sig_list_11_w_archieargweaverd), ", ", sum(!is.na(match(unique(top_phenos), unique(sig_list_11_w_archieargweaverd$id)))), " of these are supported by 3+ separately")

# add to genes_associated
region_gene_list[['11_more_maps_w_archieargweaver_regions']] = getRegionGeneAssociations(res_list_11_w_archieargweaverd)
genes_associated[['11_more_maps_w_archieargweaver_regions']] = as.data.frame(region_gene_list[['11_more_maps_w_archieargweaver_regions']])
```

### 12 methods overlapping, no archIE and Argweaverd
```{r}

# run rGREAT
res_list_12_or_more_no_archie_argweaverd = run_GREAT_GO(input_file_bed_path="/wynton/home/capra/ychen39/introgression_methods/data/GREAT_analysis/input/overlap_12_no_archie_argweaver_GREAT_merged.bed", GO_type='msigdb:C5:HPO')

# get enrichment table / sig phenotypes table
tb_12_or_more_no_archie_argweaverd = getEnrichmentTable(res_list_12_or_more_no_archie_argweaverd) #Occasionally, the output p-values will be 0 since the lowest pvalue that GREAT can return is 1E-324. Enrichments more significant than this will show a pvalue of 0.
sig_list_12_or_more_no_archie_argweaverd=tb_12_or_more_no_archie_argweaverd[tb_12_or_more_no_archie_argweaverd$p_adjust < 0.05,]

# edit names so they don't contain file path and suffix
names(sig_list_12_or_more_no_archie_argweaverd) <- sub("/", "", sub(GREAT_input_dir, "", sub("_GREAT_merged.bed$", "", names(sig_list_12_or_more_no_archie_argweaverd))))

# number of sig phenotypes
# how many of these sig phenotypes are supported by 3+ separately (top_phenos)
paste0("Number sig phenotypes:", nrow(sig_list_12_or_more_no_archie_argweaverd), ", ", sum(!is.na(match(unique(top_phenos), unique(sig_list_12_or_more_no_archie_argweaverd$id)))), " of these are supported by 3+ separately")

# add region-gene list
region_gene_list[['12_maps_regions_no_archie_argweaverd']] = getRegionGeneAssociations(res_list_12_or_more_no_archie_argweaverd)
genes_associated[['12_maps_regions_no_archie_argweaverd']] = as.data.frame(region_gene_list[['12_maps_regions_no_archie_argweaverd']])
```

### 11+ methods overlapping no archie/argweaverd, with nean background
```{r}

# run rGREAT
res_list_11_or_more_nean_background = run_GREAT_GO(input_file_bed_path="/wynton/home/capra/ychen39/introgression_methods/data/GREAT_analysis/input/overlap_11_high_support_no_archie_argweaver_GREAT_merged.bed",
                                   background_file_path_bed="/wynton/home/capra/ychen39/introgression_methods/data/GREAT_analysis/nean_background_merged.bed",
                                   GO_type='msigdb:C5:HPO')

# get enrichment table
tb_11_or_more_background = getEnrichmentTable(res_list_11_or_more_nean_background) 
sig_list_11_or_more_background = tb_11_or_more[tb_11_or_more$p_adjust < 0.05,]

# edit names so they don't contain file path and suffix
names(sig_list_11_or_more_background) <- sub("/", "", sub(GREAT_input_dir, "", sub("_GREAT_merged.bed$", "", names(sig_list_11_or_more_background))))

# number of sig phenotypes
# how many of these sig phenotypes are supported by 3+ separately (top_phenos)
paste0("Number sig phenotypes:", nrow(sig_list_11_or_more_background), ", ", sum(!is.na(match(unique(top_phenos), unique(sig_list_11_or_more_background$id)))), " of these are supported by 3+ separately")

# add region-gene list
region_gene_list[['11_maps_no_archieargweaver_regions_nean_background']] = getRegionGeneAssociations(res_list_11_or_more_nean_background)
genes_associated[['11_maps_no_archieargweaver_regions_nean_background']] = as.data.frame(region_gene_list[['11_maps_no_archieargweaver_regions_nean_background']])
```

### 14 methods overlapping - with archie and argweaverd
```{r}
# run rGREAT
res_list_14_or_more = run_GREAT_GO(input_file_bed_path="/wynton/home/capra/ychen39/introgression_methods/data/GREAT_analysis/input/overlap_14_high_support_with_archie_argweaver_GREAT_merged.bed", GO_type='msigdb:C5:HPO')

# get enrichment table / sig phenotypes table
tb_14_or_more = getEnrichmentTable(res_list_14_or_more)
sig_list_14_or_more = tb_14_or_more[tb_14_or_more$p_adjust < 0.05,]

# edit names so they don't contain file path and suffix
names(sig_list_14_or_more) <- sub("/", "", sub(GREAT_input_dir, "", sub("_GREAT_merged.bed$", "", names(sig_list_14_or_more))))

# number of sig phenotypes
# how many of these sig phenotypes are supported by 3+ separately (top_phenos)
paste0("Number sig phenotypes:", nrow(sig_list_14_or_more), ", ", sum(!is.na(match(unique(top_phenos), unique(sig_list_14_or_more$id)))), " of these are supported by 3+ separately")


# add region-gene list
region_gene_list[['14_or_more_warchieargweaver_maps_regions']] = getRegionGeneAssociations(res_list_14_or_more)
genes_associated[['14_or_more_warchieargweaver_maps_regions']] = as.data.frame(region_gene_list[['14_or_more_warchieargweaver_maps_regions']])
```
...
```{r}
rbind(sig_list_11_or_more_no_archie_argweaverd, sig_list_11_w_archieargweaverd, sig_list_12_or_more_no_archie_argweaverd, sig_list_11_or_more_background, sig_list_14_or_more)

combined_high_support <- rbind(data.frame(id = "11+ maps overlapping, no ArchIE/ArgweaverD", sig_list_11_or_more),
      data.frame(id = "11+ maps overlapping, including ArchIE/ArgweaverD", sig_list_11_w_archieargweaverd),
      data.frame(id = "All maps overlapping, no ArchIE/ArgweaverD", sig_list_12_or_more_no_archie_argweaverd),
      data.frame(id = "11+ maps overlapping, no ArchIE/ArgweaverD, with background of all maps", sig_list_11_or_more_background),
      data.frame(id = "All maps overlapping, including Archie/ArgweaverD", sig_list_14_or_more))
...
```
# Repeat bar/upset bar with 11+ methods and each map
## Bar plot: number methods supporting a phenotype
```{r}
# get unique and shared (more than one method found a pheno sig) significant phenotypes
# remove ArchIE
...
sig_list_11_or_more$method <- 'Regions supported by 11+ maps \n(no ArchIE/ArgWeaver-D)'
sig_list_11_or_more <- rbind(sig_list_11_or_more, sig_phenos)

# save this file -- will be in the supplemnt
sig_list_11_or_more
write_tsv(sig_list_11_or_more, file = paste0(HOMEDIR, '/introgression_methods/cleaned/supplement_data/rGREAT_sig_phenos_include_highsupportloci.tsv'))

# collapse multiple methods supporting a phenotype into one column
# commas separating methods
phenos_summarise <- sig_list_11_or_more %>%
  group_by(id) %>%
  arrange(p_adjust) %>%
  reframe(
    method = paste(unique(method), collapse = ", "),  # concatenate methods
...
upset_data$sprime <- 0
upset_data$vernot_2016 <- 0
upset_data$skov2018 <- 0
...
all_exon_dm_annotated <- gene_list_annotation(dir_list=methods_dirs, 
...
# and using the merged file across all methods
overlap_11_high_support_no_archie_argweaver_exon_dm_annotated <- gene_list_annotation(dir_list=c('input'), 
                     base_path=paste0(HOMEDIR, '/introgression_methods/data/GREAT_analysis/'),
                     input_bed_file='overlap_11_high_support_no_archie_argweaver_GREAT_merged.bed',
                     output_file_name='overlap_11_high_support_no_archie_argweaver_exons_genes_list.txt', 
                     annotations_to_include=c('hg19_genes_exons'))

# and using the merged file across all methods
overlap_11_high_support_with_archie_argweaver_exon_dm_annotated <- gene_list_annotation(dir_list=c('input'), 
                     base_path=paste0(HOMEDIR, '/introgression_methods/data/GREAT_analysis/'),
                     input_bed_file='overlap_11_high_support_with_archie_argweaver_GREAT_merged.bed',
                     output_file_name='overlap_11_high_support_with_archie_argweaver_exons_genes_list.txt', 
                     annotations_to_include=c('hg19_genes_exons'))

overlap_12_no_archie_argweaver_exon_dm_annotated <- gene_list_annotation(dir_list=c('input'), 
                     base_path=paste0(HOMEDIR, '/introgression_methods/data/GREAT_analysis/'),
                     input_bed_file='overlap_12_no_archie_argweaver_GREAT_merged.bed',
                     output_file_name='overlap_12_no_archie_argweaver_exons_genes_list.txt', 
                     annotations_to_include=c('hg19_genes_exons'))

overlap_14_with_archie_argweaver_exon_dm_annotated <- gene_list_annotation(dir_list=c('/input/'), 
                     base_path=paste0(HOMEDIR, '/introgression_methods/data/GREAT_analysis/'),
                     input_bed_file='overlap_14_methods_with_archieargweaver_GREAT_merged.bed',
                     output_file_name='overlap_14_with_archie_argweaver_exons_genes_list.txt', 
                     annotations_to_include=c('hg19_genes_exons'))
...
```{r}
file_info <- list(
  "IBDMix" = "li_2024/exons_genes_list.txt",
  ...
  "hmmix20" = "skov_2020/exons_genes_list.txt",
  "hmmix18" = "skov_2018/exons_genes_list.txt",
  "SARGE" = "schaefer_2021/exons_genes_list.txt",
  "ARGweaverD" = "hubisz_2020/exons_genes_list.txt",
  "Sprime" = "browning_2018_updated_filtering/exons_genes_list.txt",
  ...
  "admixfrog" = "iasi_2024/exons_genes_list.txt",
  "Overlap 11+ (without Archie ARG-weaverD)" = "overlap_11_high_support_no_archie_argweaver_exons_genes_list.txt",
  "Overlap 11+ (with Archie ARG-weaverD)" = "overlap_11_high_support_with_archie_argweaver_exons_genes_list.txt",
  "Overlap 12+ (without Archie ARG-weaverD)" = "overlap_12_no_archie_argweaver_exons_genes_list.txt",
  "Overlap (All)" = "overlap_14_with_archie_argweaver_exons_genes_list.txt"
)
...
for (method in names(file_info)) {
  if (startsWith(method, 'Overlap')) {
    file_path <- paste0(HOMEDIR, '/introgression_methods/data/GREAT_analysis/input/', file_info[[method]])
...
sig_phenos_counts
```
## EnrichR PCA - with archie or argweaverD
```{r}
# subset data of interest
...
# save plot as SVG
ggsave(paste0(HOMEDIR, 'introgression_methods/figures/introgression_tools/enrichr_pca_archieargweaver.svg'), plot = plot, width = 6, height = 4, device = "svg")
```

## EnrichR PCA - no archie or argweaverD
```{r}
# subset data of interest
subset <- enriched_by_method[c('Term', 'Adjusted.P.value', 'method')]

# remove ARGWeaver-D, archie, and all methods
subset <- subset[subset$method != "ARGWeaverD", ]
subset <- subset[subset$method != "ArchIE", ]
subset <- subset[subset$method != "All", ]
...
# save plot as SVG
ggsave(paste0(HOMEDIR, 'introgression_methods/figures/introgression_tools/enrichr_pca.svg'), plot = plot, width = 6, height = 4, device = "svg")
```
...
### 11+ methods overlapping - no archie and argweaver-d
```{r}
...
# keep entries supported by 11+ methods (15 because chrom, start, end, length)
all_methods_11_no_archie_argweaverd <- all_methods_no_archie_argweaverd[rowSums(!is.na(all_methods_no_archie_argweaverd)) == 15, ]

write.table(all_methods_11_no_archie_argweaverd, paste0(HOMEDIR, '/introgression_methods/cleaned/introgression_tools/support_11_no_archie_argweaverd.bed'), col.names=FALSE, sep='\t', row.names = FALSE,  quote = FALSE)

supported_11_no_archieargweaverd_exons_dm_annotated <- gene_list_annotation(dir_list=c('introgression_tools'), 
                     base_path=paste0(HOMEDIR, '/introgression_methods/cleaned/'),
                     input_bed_file='support_11_no_archie_argweaverd.bed', 
                     output_file_name='support_11_no_archie_argweaverd_genes_list.txt', 
                     annotations_to_include=c('hg19_genes_exons'))

# run EnrichR
# get gene list
file_path <- paste0(HOMEDIR, '/introgression_methods/cleaned/introgression_tools/support_11_no_archie_argweaverd_genes_list.txt')
exons_gene_list_11_no_archie_argweaverd <- read.table(file_path, header = TRUE, col.names = c('genes'), sep = "\t")
...
# for each method, calculate enrichment for the databases
enriched <- enrichr(exons_gene_list_11_no_archie_argweaverd$genes, dbs)
...
sig_list_11_or_more <- stacked_enriched[stacked_enriched$Adjusted.P.value<0.05, ]

sig_list_11_or_more = subset(sig_list_11_or_more, select = -c(Old.Adjusted.P.value, Old.P.value, P.value, Genes, Combined.Score) )
...
# remove archie and argweaverd
all_methods = subset(all_methods)

# keep entries supported by 11+ methods (14 because chrom, start, end, length)
all_methods <- all_methods[rowSums(!is.na(all_methods)) == 15, ]

write.table(all_methods, paste0(HOMEDIR, '/introgression_methods/cleaned/introgression_tools/support_11_with_archie_argweaverd.bed'), col.names=FALSE, sep='\t', row.names = FALSE,  quote = FALSE)

supported_11_no_archieargweaverd_exons_dm_annotated <- gene_list_annotation(dir_list=c('introgression_tools'), 
                     base_path=paste0(HOMEDIR, '/introgression_methods/cleaned/'),
                     input_bed_file='support_11_with_archie_argweaverd.bed', 
                     output_file_name='support_11_with_archie_argweaverd_genes_list.txt', 
                     annotations_to_include=c('hg19_genes_exons'))

# run EnrichR
# get gene list
file_path <- paste0(HOMEDIR, '/introgression_methods/cleaned/introgression_tools/support_11_with_archie_argweaverd_genes_list.txt')
exons_gene_list_11_no_archie_argweaverd <- read.table(file_path, header = TRUE, col.names = c('genes'), sep = "\t")
...
sig_list_11_w_archieargweaverd <- stacked_enriched[stacked_enriched$Adjusted.P.value<0.05, ]

sig_list_11_w_archieargweaverd = subset(sig_list_11_w_archieargweaverd, select = -c(Old.Adjusted.P.value, Old.P.value, P.value, Genes, Combined.Score) )

```


### 12 methods overlapping, no archie/argweaver-D
```{r}
# read in all methods bed file
all_methods <- read.table(paste0(HOMEDIR, '/introgression_methods/cleaned/introgression_tools/all_methods.bed'), sep="\t", stringsAsFactors=FALSE, header = TRUE)
#replace empty strings with NA
all_methods[all_methods==""]<-NA
all_methods_support_12 = subset(all_methods, select = -c(archie, argweaverd) )
# keep entries supported by all 12 methods (16 because chrom, start, end, length)
all_methods_support_12 <- all_methods_support_12[rowSums(!is.na(all_methods_support_12)) > 15, ]
# subset to bed
all_methods_support_12_bed <- all_methods_support_12[, c('chrom', 'start', 'end')]
all_methods_support_12_bed$chrom <- paste0('chr', all_methods_support_12_bed$chrom)

write.table(all_methods_support_12_bed, paste0(HOMEDIR, '/introgression_methods/cleaned/introgression_tools/support_12_noarchieargweaver.bed'), col.names=FALSE, sep='\t', row.names = FALSE,  quote = FALSE)

supported_11_exons_dm_annotated <- gene_list_annotation(dir_list=c('introgression_tools'), 
                     base_path=paste0(HOMEDIR, '/introgression_methods/cleaned/'),
                     input_bed_file='support_12_noarchieargweaver.bed', 
                     output_file_name='support_12_noarchieargweaver_genes_list.txt', 
                     annotations_to_include=c('hg19_genes_exons'))

# run EnrichR
# get gene list
file_path <- paste0(HOMEDIR, '/introgression_methods/cleaned/introgression_tools/support_12_noarchieargweaver_genes_list.txt')
exons_gene_list_12_methods <- read.table(file_path, header = TRUE, col.names = c('genes'), sep = "\t")
...
# for each method, calculate enrichment for the databases
enriched <- enrichr(exons_gene_list_12_methods$genes, dbs)
...
sig_list_12_or_more_no_archie_argweaverd <- stacked_enriched[stacked_enriched$Adjusted.P.value<0.05, ]

sig_list_12_or_more_no_archie_argweaverd = subset(sig_list_12_or_more_no_archie_argweaverd, select = -c(Old.Adjusted.P.value, Old.P.value, P.value, Genes, Combined.Score) )
```
...
all_methods_14 <- all_methods[rowSums(!is.na(all_methods)) == 18, ]

write.table(all_methods_12, paste0(HOMEDIR, '/introgression_methods/cleaned/introgression_tools/support_14.bed'), col.names=FALSE, sep='\t', row.names = FALSE,  quote = FALSE)

support_12_exons_dm_annotated <- gene_list_annotation(dir_list=c('introgression_tools'), 
                     base_path=paste0(HOMEDIR, '/introgression_methods/cleaned/'),
                     input_bed_file='support_14.bed', 
                     output_file_name='support_14_genes_list.txt', 
                     annotations_to_include=c('hg19_genes_exons'))

# run EnrichR
# get gene list
file_path <- paste0(HOMEDIR, '/introgression_methods/cleaned/introgression_tools/support_14_genes_list.txt')
exons_gene_list_14 <- read.table(file_path, header = TRUE, col.names = c('genes'), sep = "\t")
...
# for each method, calculate enrichment for the databases
enriched <- enrichr(exons_gene_list_14$genes, dbs)
...
sig_list_14_or_more <- stacked_enriched[stacked_enriched$Adjusted.P.value<0.05, ]

sig_list_14_or_more = subset(sig_list_14_or_more, select = -c(Old.Adjusted.P.value, Old.P.value, P.value, Genes, Combined.Score) )

```
## Generate file containing phenotypes associated with high-confidence loci
```{r}
rbind(sig_list_11_or_more, sig_list_11_w_archieargweaverd, sig_list_12_or_more_no_archie_argweaverd, sig_list_14_or_more)
...
iasi_2024_original_deserts <- read.table(paste0(HOMEDIR, '/introgression_methods/cleaned/introgression_tools/iasi_2024/original_deserts.bg'), header = FALSE, sep="\t",stringsAsFactors=FALSE, col.names=c("chr", "start", "end"))
...
iasi_2024_original_deserts$chr <- paste0("chr", iasi_2024_original_deserts$chr)
...
kpPlotRegions(kp, data=iasi_2024_original_deserts, col="red", r0=r0s[6], r1=r1s[6])
...
## rGREAT on regions in different clusters
```{r}
# run rGREAT
res_list_cluster1 = run_GREAT_GO(input_file_bed_path="/wynton/home/capra/ychen39/introgression_methods/cleaned/introgression_tools/cluster_analysis/cluster1_unique.bed",
                                               GO_type='msigdb:C5:HPO')

# get enrichment table / sig phenotypes table
tb_cluster1 = getEnrichmentTable(res_list_cluster1) #Occasionally, the output p-values will be 0 since the lowest pvalue that GREAT can return is 1E-324. Enrichments more significant than this will show a pvalue of 0.
sig_list_cluster1 = tb_cluster1[tb_cluster1$p_adjust < 0.05,]
