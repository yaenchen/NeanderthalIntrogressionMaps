...
def save_bed_only(df=False, full_file_path=False, output_file_path=False, colnames=False, prefix=False, save_with_header=False, stratify=False, output_dir=False, ancestry=False):
    ...
    headers: list of column names, or False if already in the data
    ...
    output_dir: output directory to store stratified .bed files
...
def chen_2020_raw_to_bed(input_path, output_dir, output_file_name, population=False, genomes_source=False, individual=False):
    ...
    Args:
        ...
        genomessource: the project that the genomes are from (usually 1000 Genomes Project (1KG) or Simons Genome Diversity Project (SGDP))
...
def durvasula_2019_raw_to_bed(output_dir, output_file_name, ancestry, input_dir=None, genomes_source=None, file_path=None, population=False, individual=False, threshold=0.892, keep_old_score=False, keep_average_score=False):
    ...
    Args:
        ...
        population: true if to be cleaned at the population level, input_path file should already be cleaned at the individual level
        individual: true if only to be cleaned at the individual level
        threshold: min threshold (Archaic probability) to be considered introgressed
...
def skov_2020_raw_to_bed(input_file, output_dir, output_file_name, archaic, population=False, ancestry='Icelandic', liftover_path=None, genomes_source='Icelandic Genomes', snp_level=False):
    ...
    if snp_level:
        ...
        # add header to union_merged.bg
        #os.system(f'cd {output_dir}/bedgraphs; echo "chrom  start   end {ancestry}" | cat - union_merged.bg > temp && mv temp union_merged.bg')
        # completed
        print(f'Output .bed file is located at {save_path}', sep='')

def schaefer_2021_raw_to_bed(input_file, output_dir, output_file_name, archaic, population=None, individual=None, genomes_source='SGDP', input_path=None):
    """
    ...
    Args:
        ...
        genomessource: the source that the genomes are from (usually 1000 Genomes Project (1KG) or Simons Genome Diversity Project (SGDP))
        population: true if to be cleaned at the population level, input_path file should already be cleaned at the individual level
        individual: true if only to be cleaned at the individual level
    """
...
def browning_2018_raw_to_bed(input_dir, output_dir, output_file_name, archaic, genomes_source='1KG Phase 3 version 5a', population=True, snp_level=False):
    ...
    Args:
        ...
        snps: true if you want to save the variants, not the fragments
    """
    def create_bed_format(df):
        ...
        # Rename the 'POS' column to 'End' and change datatype to int
        #df.rename(columns={'POS':'End'}, inplace=True)
        df['End'] = df['End'].astype(int)
        # Create the 'Start' column by subtracting 1 from the 'End' positions
        df.insert(1, 'Start', df['End']-1)
        
        return df

    def sort_chromosome(df):
        # To prevent the lexicographic sorting of 'Chr'
        # Define the sorting order for 'Chr'
    ...
    # Sort the rows by locus
    snps = snps.sort_values(by=['Chr','End'])
    # Wrangle the dataframe into bed format
    ...
    if archaic=='Denisovan':
        ...
        snps_final = snps_final.drop(['DMatch', 'NMatch'], axis=1)
    # Collapse the 'Population' column and sort
...
# def vernot_2016_raw_to_bed_snp(snp_input_file, extended_ld_input_file, hap_input_file, output_dir, output_file_name, archaic, genomes_source='1KG and Melanesian samples'):
#     """
...
#         Args:
#             input_file is the output of vernot_2016_raw_to_bed.sh, containing SNP and haplotype-level data from extended LD and median extended LD files.
#                 columns: 'Chromosome', 'Start', 'Stop', 'AncestralAllele', 'DerivedAllele', 'NeanderthalBase', 'DenisovaBase', 'HaplotypeTag', 'Population'
#             extended_ld_input_file: output of vernot_2016_raw_to_bed.sh, generated from extended LD files
#             hap_input_file: output of vernot_2016_raw_to_bed.sh, generated from median extended LD files
#                 columns: 'Chromosome', 'Start', 'End', 'ScoreName', 'Length', 'Ancestry'
#             output_dir: output directory to place the cleaned file 
#             output_file_name: output file name to place in the output_dir
#             archaic: which archaic species to include ('Neanderthal' or 'Denisovan')
#             genomessource: the source that the genomes are from (usually 1000 Genomes Project (1KG) or Simons Genome Diversity Project (SGDP))
...
#     vernot_2016_snp = pd.read_csv(f'{snp_input_file}', sep='\t')
#     # Read in the SNP only file and stratify by Neanderthal vs. Denisovan SNPs
#     # separate by archaic hominim
#     if archaic=='Neanderthal':
#         vernot_2016_snps = vernot_2016_snp[vernot_2016_snp.apply(lambda row: row['DerivedAllele'] in row['NeanderthalBase'], axis=1)]
#     if archaic=='Denisovan':
#         vernot_2016_snps = vernot_2016_snp[vernot_2016_snp.apply(lambda row: row['DerivedAllele'] in row['DenisovaBase'], axis=1)]

#     # Read in the SNPs + LD SNPs file and keep only the haplotype IDs from the Nean/Den files
#     vernot_2016_snps_ld = pd.read_csv(f'{extended_ld_input_file}', sep='\t')
#     vernot_2016_snps_extended_ld = vernot_2016_snps_ld[vernot_2016_snps_ld['HaplotypeTag'].isin(vernot_2016_snps['HaplotypeTag'])]

#     # Read in the haplotypes (median AF file), positions correspond to previous haplotype IDs
#     vernot_2016_haps = pd.read_csv(f'{hap_input_file}', sep='\t')
#     vernot_2016_haps.rename(columns={'Stop': 'End'}, inplace=True)
#     # Generate haplotype tags for the rows in vernot_2016_haps
#     vernot_2016_haps['HaplotypeTag'] = vernot_2016_haps.apply(lambda row: '_'.join(str(row[i]) for i in ['Chromosome', 'Start', 'End']), axis=1)
#     # Find matching haplotype tags from ld SNPs in vernot_2016_snp_nean
#     mask = vernot_2016_haps['HaplotypeTag'].isin(vernot_2016_snps_extended_ld['HaplotypeTag'])
#     vernot_2016_haps = vernot_2016_haps[mask]
    
#     # rename populaiton to ancestry
#     vernot_2016_haps.rename(columns={'Population': 'Ancestry'}, inplace=True)
#     # renames SNPsOnHaplotype to Score
#     vernot_2016_haps.rename(columns={'SNPsOnHaplotype': 'Score'}, inplace=True)
...
def plot_jaccard(boolean_df, categories_list, value_column, type='combined', leaf_order=False, region_level=False, combined_title=False, save_fig_path=False, color='Reds'):
    ...
    leaf_order: order by jaccard values
    ...
    if type == 'raw':
        ...
        if type == 'normalized': # plot norm jaccard if specified
            ...
            heatmap = sns.heatmap(jaccard_heatmap_combined.astype(float), annot=True, cmap=color, vmin=0, vmax=1)
...
def plot_cumulative_increase(boolean_overlap_df, fig_save_path=False, region_level=False, title=False, xtitle=False, ytitle=False, rename_methods=False, color_by_pops=False):
    ...
    # Initialize an empty filter (all False initially)
    cumulative_filter = pd.Series([False] * len(boolean_overlap_df))

    # Iterate through each column and progressively add it to the filter
    for col in columns:

        # get the ancestral group from the original df
        ancestry = df[df['ID'] == col]['Ancestry']
        
        # Update the filter by adding the current column
        new_filter = (boolean_overlap_df[col] == True)

        # Combine with the cumulative filter (keep track of the new True values added)
        combined_filter = cumulative_filter | new_filter

        # Compute the sum of 'length' for the newly added True values
        length_sum = boolean_overlap_df[combined_filter]['length'].sum()

        # Append the result with the column name and new length sum
        length_sums.append((col, length_sum, ancestry))

        # Update the cumulative filter to include the current column
...
def subset_bedgraph_by_score(input_file, length_threshold, output_file=False, autosomes=False, exact=False, x_only=False):
    ...
    # Select regions that fully fit within the threshold
    subset = df_sorted[df_sorted['cumsum_length'] <= length_threshold].copy()
    # if the first entry has a greater length than the threshold, handle it separately
    if len(subset)==0:
        subset = df_sorted.head(1)

    if exact == True:

        # Calculate the remaining length needed to reach the threshold
        remaining_length = length_threshold - subset['cumsum_length'].max()
        
        # If there is a remaining length, keep adding rows until we reach the threshold
        if remaining_length > 0:
            ...
            while index < len(df_sorted) and remaining_length > 0:
                ...
                # If the next row is larger than needed, trim it
                if next_row['length'] > remaining_length:
                    ...
                    # Otherwise, add the whole row and update the remaining length
                    remaining_length -= next_row['length']
    
                # Add the selected row to the subset
                subset = pd.concat([subset, next_row.to_frame().T], ignore_index=True)
                index += 1  # Move to the next row
        # EXCEPTION: if less than 0 (need to subtract from the first entry first entry)
        elif remaining_length < 0:
            # the remaining length is neg length_threshold
    ...
    if output_file != False:
        # Save the subset to a new BEDGraph file
...
def individual_jaccard_comparison(dfs_dict, temp_dir, output_jaccards_file_path, autosomes_only, x_only, save_shared_individuals_bed=False, length_threshold=False, return_heatmap=False):
    ...
    # For each ID, calculate Jaccard (stored in a dictionary initialized below)
...
def individual_jaccard_comparison_heatmap(tsv, title, color, save_plot_path, figsize_tuple=(8, 6)):
    ...
    # Set new labels excluding the first y-label and last x-label
...
def individual_jaccard_comparison_dot_heatmap(heatmap_data, tsv, title, color, save_plot_path, figsize_tuple=(8, 6), text_position=0.2):
    '''
    ...
    # Make sure column names match up, reorder
    ...
    # Pivot for heatmap, also reorder by distance
    ...
    # Overlay mean values (shifted upward)
    ...
    # Overlay scatter plot for SD Jaccard (shifted downward)
    ...
    legend_handles = [plt.scatter([], [], s=s * 5000, color='black', alpha=0.6, edgecolors='white', label=f'SD: {s:.5f}') for s in sizes]
